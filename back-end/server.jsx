...
app.post("/signup", (req, res) => {
  ...
  // if (typeof phoneOne === 'Number' && (typeof phoneTwo === 'Number' || phoneTwo==='')){
  const queryOne =
    "INSERT INTO passenger_details (first_name, last_name, email, password, nationality, passport_no, gender, age, address) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)";
  mysqlConnection.query(
    queryOne,
    [
      firstname,
            ...
            if (!err) {
              ...
              mysqlConnection.query(
                ...
                (err, mobiledata) => {
                  ...
                  } else console.log("MOBILE TABLE ERROR" + err);
                }
              );
                ...
                mysqlConnection.query(
                  ...
                  (err, mobiledata) => {
                    if (!err) {
                      res.json({ message: "Data Added" });
                      console.log("DATA ADDED TO THE DATABASE SUCCESSFULLY");
                    } else console.log("MOBILE TABLE 2 ERROR" + err);
                  }
                );
              }
            } else console.log("PASSENGER TABLE ERROR" + err);
          }
        );
      } else {
...
app.post("/signin", (req, res) => {
  ...
  mysqlConnection.query(queryOne, [email, password], (err, data) => {
    if (!err) {
      if (data.length > 0) {
        const newEmail = data[0].email; // Retrieve the email from the first row of the result
        ...
        if (newPass === password) {
          const queryMobile = `SELECT t3.mobile_no
        ...
        .then((exists) => {
          if (exists) {
            ...
            mysqlConnection.beginTransaction((err) => {
              ...
              const flightCheckQuery =
                "INSERT INTO checks_flight (passenger_id, flight_no, flight_class) VALUES (?, ?, ?)";
              mysqlConnection.query(
                flightCheckQuery,
                [p_id, f_no, fclass],
                (err, result) => {
                  if (err) {
                    ...
                    const ticketQuery =
                      "INSERT INTO ticket (ticket_no, class, seat_no) VALUES (?, ?, ?)";
                    mysqlConnection.query(
                      ticketQuery,
                      [ticket_no, fclass, seatno],
                      (err, result) => {
                        if (err) {
                          console.log(3, err);
                          ...
                          const bookedQuery =
                            "INSERT INTO booked_flight (ticket_no, flight_no) VALUES (?, ?)";
                          mysqlConnection.query(
                            bookedQuery,
                            [ticket_no, f_no],
                            (err, result) => {
                              if (err) {
                                ...
                                const ticketBookQuery =
                                  "INSERT INTO ticket_booked (passenger_id, ticket_no, booking_datetime) VALUES (?, ?, ?)";
                                mysqlConnection.query(
                                  ticketBookQuery,
                                  [p_id, ticket_no, booking_datetime],
                                  (err, result) => {
                                    if (err) {
                                      console.log(5);
                                      mysqlConnection.rollback(() => {
                                      ...
                                      const paymentQuery =
                                        "INSERT INTO payment (transaction_id, status, account_no) VALUES (?, ?, ?)";
                                      mysqlConnection.query(
                                        paymentQuery,
                                        [transaction_id, status, account_no],
                                        (err, result) => {
                                          if (err) {
                                            console.log(6);
                                            mysqlConnection.rollback(() => {
                                            ...
                                            const receivePaymentQuery =
                                              "INSERT INTO receives_payment (ticket_no, transaction_id, amount) VALUES (?, ?, ?)";
                                            mysqlConnection.query(
                                              receivePaymentQuery,
                                              [
                                                ticket_no,
